...
public class CPTSMachine {
	...
	public static boolean addStation(String name) {
		...
		Station station = new Station(name,totalStationNumber);
		try {
	...
	public static boolean buyTicket(int type, Station start, Station end) {
		...
		if (start.equals(end) || start.getName().equals(end.getName())) {
			System.out.println("This ticket cannot be bought.");
			return false;
		}
		if((isStationExisted(start.getName()) && isStationExisted(end.getName()))) {
			/* FILL CODE */
			int distance = Math.abs(start.getNumber() - end.getNumber());
			if (type == 2 && distance > 6){
				System.out.println("This ticket cannot be bought.");
				return false;
			}
			...
			switch (newTicket.getType()) {
				case 0:
					passagerType = "Student Ticket";
					break;
				case 1:
					passagerType = "Adult Ticket";
					break;
				case 2:
					passagerType = "Elderly Ticket";
					break;
			}
			System.out.println("Bought "+ passagerType + ", from " + start.getName() + " to " + end.getName() +", for "+newTicket.calculatePrice()+" Baht!");
			return true;
		}else {
			System.out.println("This ticket cannot be bought.");
			return false;
